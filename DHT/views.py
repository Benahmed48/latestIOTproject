...
def sendtele(dht):
    token = '6868435984:AAFqEcdpknN0YcoO7xP_JP-wAXY0i_T9Gfw'
    rece_id = 5722868698
    bot = telepot.Bot(token)
    message = f' 🔔 Message Envoyer par Bahae: la température dépasse la normale 🔔 \n'
    message += f'🔥TEMP: {dht.temp} 💧HUM: {dht.hum} ⏰Date: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
    bot.sendMessage(rece_id, message)

def sendwhatsap():
    # Alertwhatsapp
    account_sid = 'AC4584140490172aa96872bf4d80bfbf65'
    auth_token = '4cf59fbb20a96cf0253229c5e81a3a21'
    client = Client(account_sid, auth_token)
    message = client.messages.create(
        from_='whatsapp:+14155238886',
        body='🔔 la température dépasse la normale 🔔',
        to='whatsapp:+212630408656'
    )
